...
pub struct Args {
    ...
    #[arg(long)]
    pub exclude_crate_path: Option<Vec<String>>,
    ...
    #[arg(long)]
    pub keep_dep_kinds: Option<dep_kinds_filtering::DepKinds>,
    ...
    /// Write a JSON report of which crates were kept and which were replaced
    /// with stubs to this path.
    ///
    /// Consumers that need the filtered set (to generate an SBOM, for example)
    /// can read it from here instead of inspecting the vendor directory.
    #[arg(long, value_name = "PATH")]
    pub filter_report: Option<Utf8PathBuf>,
...
/// A crate in the vendor directory, as reported by `--filter-report`.
...
/// The set of crates kept and stubbed, written by `--filter-report`.
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
pub struct FilterReport {
    /// The format of this file; bumped when the shape changes incompatibly.
    pub version: u32,
    pub kept: Vec<CrateRef>,
    pub stubbed: Vec<CrateRef>,
}

/// The current `--filter-report` format.
pub const FILTER_REPORT_VERSION: u32 = 1;
...
///
/// Returns the crate that was stubbed.
fn replace_with_stub(path: &Utf8Path) -> Result<CrateRef> {
    ...
    let stubbed = stubbed_crate_ref(&cargo_toml_data)
        .with_context(|| format!("Reading package identity from {cargo_toml_path}"))?;
...
/// Reads the name and version out of a vendored crate's manifest.
fn stubbed_crate_ref(manifest: &toml::Value) -> Result<CrateRef> {
    let package = manifest
        .get(MANIFEST_KEY_PACKAGE)
        .and_then(|p| p.as_table())
        .ok_or_else(|| anyhow!("Manifest has no [package] section"))?;
    let field = |key: &str| {
        package
            .get(key)
            .and_then(|v| v.as_str())
            .map(ToOwned::to_owned)
            .ok_or_else(|| anyhow!("Manifest [package] has no {key}"))
    };
    Ok(CrateRef {
        name: field("name")?,
...
fn delete_unreferenced_packages(
    ...
    let mut kept: Vec<CrateRef> = package_filenames
        .values()
        .map(|p| CrateRef {
...
pub fn run(args: Args) -> Result<()> {
    ...
    let report = delete_unreferenced_packages(&output_dir, &package_filenames, &excludes)?;
    if let Some(report_path) = args.filter_report.as_deref() {
